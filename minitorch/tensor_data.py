...
def to_index(ordinal: int, shape: Shape, out_index: Union[OutIndex, List[int]]) -> None:
    ...
    for i in range(len(shape) - 1, 0, -1):
        contiguous_strides.insert(0, shape[i] * contiguous_strides[0])
...
def broadcast_index(
    ...
    for i in range(-1, -1 * len(shape) - 1, -1):
        if shape[i] == 1:
            # this index in the big shape related to the other shape given for broadcasting, so just 0 here
            out_index[i] = 0
        else:
...
def shape_broadcast(shape1: UserShape, shape2: UserShape) -> UserShape:
    ...
    """
    final_len = max(len(shape1), len(shape2))
    shape1 = [1] * (final_len - len(shape1)) + list(shape1)
    shape2 = [1] * (final_len - len(shape2)) + list(shape2)
    res = []
